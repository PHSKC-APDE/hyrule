...
1.  [Record Linkage Pipeline](_targets.md): A mostly comprehensive example of a `targets` analysis pipeline for machine learning record linkage. The [origin \_targets.qmd](_targets.qmd) file uses targets-flavored markdown and can be edited (or stripped for parts) into a record linkage pipeline using "real" data.
